...
/**
 * Create a demo application 创建演示应用程序
 */
...
static void write_create(lv_obj_t *parent)
{
    ...
    style_kb_rel.body.main_color = LV_COLOR_HEX3(0x333);    /*Recommended if LV_VDB_SIZE == 0 and bpp > 1 fonts are used 推荐使用LV_VDB_SIZE == 0和bpp > 1字体*/
...
{
    lv_obj_t * parent = lv_obj_get_parent(lv_obj_get_parent(ta));   /*Test area is on the scrollabe part of the page but we need the page itself 测试区域位于页面的scrollabe部分，但是我们需要页面本身*/
...
/**
 * Called when the close or ok button is pressed on the keyboard 当按下键盘上的“关闭”或“确定”按钮时调用
 * @param keyboard pointer to the keyboard 键盘指针指向键盘
 * @return
 */
static lv_res_t keyboard_hide_action(lv_obj_t * keyboard)
{
...
static void list_create(lv_obj_t *parent)
{
    ...
    /*Create styles for the buttons 为按钮创建样式*/
    ...
    lv_mbox_add_btns(mbox, mbox_btns, NULL);    /*The default action is close 默认操作是关闭的*/
...
static void chart_create(lv_obj_t *parent)
{
    ...
    /*Create a bar, an indicator and a knob style创建一个条形图、一个指示器和一个旋钮样式*/
    ...
    /*Create a second slider 创建第二个滑块*/
    ...
    lv_obj_align(slider, chart, LV_ALIGN_OUT_BOTTOM_MID, 0, (LV_VER_RES - chart->coords.y2 - lv_obj_get_height(slider)) / 2); /*Align to below the chart 对齐到图表下方*/
    lv_slider_set_action(slider, slider_action);
    lv_slider_set_range(slider, 10, 1000);
    lv_slider_set_value(slider, 700);
    slider_action(slider);          /*Simulate a user value set the refresh the chart 模拟用户值设置刷新图表*/
	#else
	    /*Create a Label on the currently active screen 在当前活动屏幕上创建一个标签*/
    lv_obj_t * label1 =  lv_label_create(parent, NULL);

    /*Modify the Label's text 修改标签的文本*/
    lv_label_set_text(label1, "Hello world!");

    /* Align the Label to the center 将标签对齐到中心
     * NULL means align on parent (which is the screen now) NULL表示对齐到父节点上(现在是屏幕)
     * 0, 0 at the end means an x, y offset after alignment 0,0表示对齐后的x, y偏移量*/
    lv_obj_align(label1, NULL, LV_ALIGN_CENTER, 0, 0);
	#endif
}

/**
 * Called when a new value on the slider on the Chart tab is set 在设置图表选项卡上的滑块上的新值时调用
 * @param slider pointer to the slider 滑块指向滑块的指针
 * @return LV_RES_OK because the slider is not deleted in the function LV_RES_OK，因为函数中没有删除滑块
 */
static lv_res_t slider_action(lv_obj_t *slider)
{
    int16_t v = lv_slider_get_value(slider);
    v = 1000 * 100 / v; /*Convert to range modify values linearly 转换为范围修改值线性*/
...
/**
 * Called when a a list button is clicked on the List tab 在“列表”选项卡上单击“列表”按钮时调用
 * @param btn pointer to a list button 指向列表按钮的btn指针
 * @return LV_RES_OK because the button is not deleted in the function  LV_RES_OK，因为函数中没有删除按钮
 */
...
#if LV_DEMO_SLIDE_SHOW
/**
 * Called periodically (lv_task) to switch to the next tab 定期调用(lv_task)切换到下一个选项卡
 */
static void tab_switcher(void * tv)
{
