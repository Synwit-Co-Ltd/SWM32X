...
/**
 * Draw an image  绘制图像
 * @param coords the coordinates of the image 协调图片的坐标位置
 * @param mask the image will be drawn only in this area 图像掩码会在这个区域
 * @param map_p pointer to a lv_color_t array which contains the pixels of the image map_p指针指向lv_color_t数组，该数组包含图像的像素
 * @param opa opacity of the image (0..255) 图像的不透明度(0..255)
 */
             ...
             const lv_style_t * style, const void * src)
{
    ...
    if(u8_p[0] >= 'A' &&  u8_p[0] <= 'Z') { /*It will be a path of a file 它是一个文件路径的名称*/
        ...
        if(res == LV_FS_RES_OK) {
            ...
            lv_area_t mask_com;    /*Common area of mask and cords 常见的和掩码公共区域*/
            bool union_ok;
            ...
            if(img_data.header.alpha_byte) {    /*Correction with the alpha byte修正阿尔法（透明度的阿尔法）字节*/
                px_size++;
                if(img_data.header.format == LV_IMG_FORMAT_FILE_RAW_RGB888) px_size--; /*以四字节位单位存储*/
            }


            /* Move the file pointer to the start address according to mask 根据掩码将文件指针移动到起始地址 */
            uint32_t start_offset = sizeof(img_data.header);
            start_offset += (lv_area_get_width(coords)) * (mask_com.y1 - coords->y1) * px_size;      /*First row 第一行*/
            start_offset += (mask_com.x1 - coords->x1) * px_size;                                    /*First col 第一列*/
...
# endif
        ...
        lv_area_t mask_com;    /*Common area of mask and coords常见的和掩码公共区域*/
        bool union_ok;
        union_ok = lv_area_union(&mask_com, mask, coords);
        if(union_ok == false) {
            return;         /*Out of mask 返回掩码回去*/
        }
