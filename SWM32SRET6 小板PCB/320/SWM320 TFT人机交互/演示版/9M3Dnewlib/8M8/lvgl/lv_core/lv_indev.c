...
#if LV_INDEV_READ_PERIOD != 0
/**
 * Called periodically to handle the input devices 定期调用，处理输入设备
...
static void indev_proc_task(void * param)
{
    ...
    /*Read and process all indevs 读取并处理所有indevs*/
    while(i) {
        indev_act = i;

        /*Handle reset query before processing the point 在处理点之前处理重置查询*/
        ...
        if(i->proc.disabled == 0) {
            ...
            do {
                ...
                /*Handle reset query if it happened in during processing 如果在处理过程中发生重置查询*/
                indev_proc_reset_query_handler(i);
            } while(more_to_read);
        }
        i = lv_indev_next(i);    /*Go to the next indev 去下一个indev*/
    }

    indev_act = NULL;   /*End of indev processing, so no act indev结束indev处理，没有行为indev*/
}
