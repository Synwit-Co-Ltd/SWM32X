...
/**
 * Mark the object as invalid therefore its current position will be redrawn by 'lv_refr_task' 将对象标记为无效 然后重新以lv_refr_task绘制
 * @param obj pointer to an object 指向对象的obj指针
 */
void lv_obj_invalidate(lv_obj_t * obj)
{
	if(lv_obj_get_hidden(obj)) return;

    /*Invalidate the object only if it belongs to the 'act_scr' 只有当对象属于“act_scr”时才使该对象无效*/
    lv_obj_t * obj_scr = lv_obj_get_screen(obj);
    if(obj_scr == lv_scr_act() ||
       obj_scr == lv_layer_top() ||
       obj_scr == lv_layer_sys())
    {
        /*Truncate recursively to the parents递归的截断到父类*/
        lv_area_t area_trunc;
        lv_obj_t * par = lv_obj_get_parent(obj);
        bool union_ok = true;
        /*Start with the original coordinates 从原始坐标开始刷取显示*/
        ...
        /*Check through all parents检查所有父类*/
        while(par != NULL) {
            union_ok = lv_area_union(&area_trunc, &area_trunc, &par->coords);
            if(union_ok == false) break; 		/*If no common parts with parent break 如果没有与父组件共用的部件断裂;*/
        	if(lv_obj_get_hidden(par)) return;	/*If the parent is hidden then the child is hidden and won't be drawn 如果父节点被隐藏，则子节点被隐藏，并且不会被绘制*/
...
/**
 * Set relative the position of an object (relative to the parent) 设置对象的相对位置(相对于父对象)
 * @param obj pointer to an object 指向对象的 obj指针
 * @param x new distance from the left side of the parent x父节点左侧的新距离
 * @param y new distance from the top of the parent y与父节点顶部的新距离
 */
...
/**
 * Set the size of an object 设置对象大小
 * @param obj pointer to an object 指向对象的OBJ指针
 * @param w new width  设置一个新w宽度
 * @param h new height 设置一个新高度
 */
void lv_obj_set_size(lv_obj_t * obj, lv_coord_t w, lv_coord_t h)
{

    /* Do nothing if the size is not changed 如果没有更改大小，则不执行任何操作 */
    /* It is very important else recursive resizing can 这是非常重要的，否则递归调整大小可以
     * occur without size change 发生时大小没有变化*/
	if(lv_obj_get_width(obj) == w && lv_obj_get_height(obj) == h) {
		return;
	}

    /*Invalidate the original area 使原来的区域无效*/
    lv_obj_invalidate(obj);
    
    /*Save the original coordinates 保存原始坐标*/
    lv_area_t ori;
    lv_obj_get_coords(obj, &ori);
    
    //Set the length and height 设置长度和高度
    obj->coords.x2 = obj->coords.x1 + w - 1;
    obj->coords.y2 = obj->coords.y1 + h - 1;


    /*Send a signal to the object with its new coordinates 向具有新坐标的对象发送一个信号*/
    obj->signal_func(obj, LV_SIGNAL_CORD_CHG, &ori);
    
    /*Send a signal to the parent too 也向父节点发送一个信号*/
    lv_obj_t * par = lv_obj_get_parent(obj);
    if(par != NULL) par->signal_func(par, LV_SIGNAL_CHILD_CHG, obj);
    
    /*Invalidate the new area使新区域无效*/
    lv_obj_invalidate(obj);
}
...
/**
 * Return with a pointer to the active screen 返回一个指向活动屏幕的指针
 * @return pointer to the active screen object (loaded by 'lv_scr_load()') 返回由lv——scr——load读取的指向活动屏幕的指针 
 */
...
/**
 * Return with the top layer. (Same on every screen and it is above the normal screen layer)返回顶层。(每个屏幕都是一样的，在正常屏幕层之上)
 * @return pointer to the top layer object  (transparent screen sized lv_obj)返回顶层对象指针(透明屏幕大小lv_obj)
 */
...
/**
 * Return with the system layer. (Same on every screen and it is above the all other layers) 返回系统层。(每个屏幕都是一样的，它在所有其他图层之上)
 * It is used for example by the cursor 例如游标使用它
 * @return pointer to the system layer object (transparent screen sized lv_obj) 返回指向系统层对象的指针(透明屏幕大小lv_obj)
 */
...
/**
 * Return with the screen of an object 返回一个对象的屏幕
 * @param obj pointer to an object 指向对象的obj指针
 * @return pointer to a screen 返回指向屏幕的指针
 */
...
/**
 * Returns with the parent of an object 返回对象的父类
 * @param obj pointer to an object 指向对象的obj指针
 * @return pointer to the parent of  'obj'  返回指向obj父类的指针
 */
...
/**
 * Copy the coordinates of an object to an area 将对象的坐标复制到一个区域
 * @param obj pointer to an object 指向对象的obj指针
 * @param cords_p pointer to an area to store the coordinates cords_p指针指向一个存储坐标的区域
 */
...
/**
 * Get the width of an object 获取对象的宽度
 * @param obj pointer to an object 指向对象的obj指针
 * @return the width 返回宽度
 */
...
/**
 * Get the height of an object 获得高度
 * @param obj pointer to an object 指向对象的obj指针
 * @return the height 返回高度
 */
...
/**
 * Get the style pointer of an object (if NULL get style of the parent) 获取对象的样类指针 （如果为空，则获取父对象的样式）
 * @param obj pointer to an object 指向对象的obj指针
 * @return pointer to a style 返回指向样式的指针
 */
...
#else
                    /*Is a parent is focused then use then focused style 是父类的焦点，然后使用焦点样式*/
...
/**
 * Get the hidden attribute of an object 获取对象的隐藏属性
 * @param obj pointer to an object 指向对象的obj指针
 * @return true: the object is hidden 对象被隐藏
 */
...
/**
 * Get the ext pointer 获取ext指针
 * @param obj pointer to an object 指向对象的obj指针
 * @return the ext pointer but not the dynamic version 返回EXT指针 但不返回动态版本
 *         Use it as ext->data1, and NOT da(ext)->data1 使用它作为ext->data1，而不是da(ext)->data1
 */
