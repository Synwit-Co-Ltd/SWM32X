...
/**
 * Set the pixel map to display by the image 设置图像的像素映射为显示
 * @param img pointer to an image object  img指向图像对象的指针
 * @param data the image data 数据图像数据
 */
void lv_img_set_src(lv_obj_t * img, const void * src_img)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        ext->w = size.x;
        ext->h = size.y;
        ext->chroma_keyed = 1;    /*Symbols always have transparent parts, Important because of cover check in the design function 符号总是有透明的部分，重要的是由于盖检入的设计功能*/

        /* If the new and the old src are the same then it was only a refresh 如果新的src和旧的src是相同的，那么这只是一个刷新.*/
        if(ext->src != src_img) {
            lv_mem_free(ext->src);
...
/**
 * Get the type of an image source 获取图像源的类型
 * @param src pointer to an image source: 指向图像源的src指针
 *  - pointer to an 'lv_img_t' variable (image stored internally and compiled into the code) -指向“lv_img_t”变量的指针(图像存储在内部并编译到代码中)
 *  - a path to an file (e.g. "S:/folder/image.bin")  -文件的路径(例如“史:/文件夹/ image.bin”)
 *  - or a symbol (e.g. SYMBOL_CLOSE) -或符号(例如SYMBOL_CLOSE)
 * @return type of the image source LV_IMG_SRC_VARIABLE/FILE/SYMBOL/UNKOWN 返回图像源LV_IMG_SRC_VARIABLE/FILE/SYMBOL/UNKOWN的类型
 */
lv_img_src_t lv_img_get_src_type(const void * src)
{
    ...
    /*The first byte shows the type of the image source第一个字节显示了图像源的类型*/
    if(u8_p[0] >= 'A' && u8_p[0] <= 'Z') return LV_IMG_SRC_FILE;    /*It's a driver letter 驱动的回传来信*/
    else if(((u8_p[0] & 0xFC) >> 2) == LV_IMG_FORMAT_INTERNAL_RAW) return LV_IMG_SRC_VARIABLE;      /*Mask the file format part og of lv_img_t header. IT should be 0 which means C array屏蔽lv_img_t头文件格式部分og。应该是0，也就是C数组 */
    else if(u8_p[0] >= ' ') return LV_IMG_SRC_SYMBOL;               /*Other printable characters are considered symbols其他可打印字符被认为是符号*/
...
/**
 * Get the auto size enable attribute 获取auto size enable属性
 * @param img pointer to an image img指向图片的指针
 * @return true: auto size is enabled, false: auto size is disabled  true:启用自动大小，false:禁用自动大小
 */
...
/**
 * Handle the drawing related tasks of the images 处理图像绘制相关任务（非常重要）
 * @param img pointer to an object 指向对象的指针 
 * @param mask the object will be drawn only in this area 掩码会在这个区域被处理
 * @param mode LV_DESIGN_COVER_CHK: only check if the object fully covers the 'mask_p' area 模式LV_DESIGN_COVER_CHK:只检查对象是否完全覆盖了“mask_p”区域
 *                                  (return 'true' if yes)(如果是，返回“true”)
 *             LV_DESIGN_DRAW: draw the object (always return 'true') LV_DESIGN_DRAW:绘制对象(始终返回“true”)
 *             LV_DESIGN_DRAW_POST: drawing after every children are drawn LV_DESIGN_DRAW_POST:绘制完所有子元素后再进行绘制
 * @param return true/false, depends on 'mode' 返回true/false，取决于“mode”      
 */
static bool lv_img_design(lv_obj_t * img, const lv_area_t * mask, lv_design_mode_t mode)
{
    lv_style_t * style = lv_obj_get_style(img);
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
		...
		if(ext->src_type == LV_IMG_SRC_FILE || ext->src_type == LV_IMG_SRC_VARIABLE) {
		    ...
		    /*Trigger the error handler of image drawer触发图像折叠项的错误处理程序*/
