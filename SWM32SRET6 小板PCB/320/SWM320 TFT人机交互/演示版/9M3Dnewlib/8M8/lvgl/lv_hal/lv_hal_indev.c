...
/**
 * Get the next input device. 获取下一个输入设备。
 * @param indev pointer to the current input device. NULL to initialize. 指向当前输入设备的指针indev。零初始化。
 * @return the next input devise or NULL if no more. Give the first input device when the parameter is NULL 返回下一个输入设备，如果没有，返回NULL。当参数为空时，给出第一个输入设备
 */
