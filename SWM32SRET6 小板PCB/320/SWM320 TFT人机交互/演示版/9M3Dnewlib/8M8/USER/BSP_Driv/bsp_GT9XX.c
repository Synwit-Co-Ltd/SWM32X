...
static int I2C_Transfer( struct i2c_msg *msgs,int num)
{
	...
	for (im = 0; ret == 0 && im != num; im++)
	{
		...
		{
			ret = bsp_GT9XX_RdReg(msgs[im].addr, msgs[im].buf, msgs[im].len);		//IIC读取数据
		} else
		{
			ret = bsp_GT9XX_WrReg(msgs[im].addr,  msgs[im].buf, msgs[im].len);		//IIC写入数据
		}
	}
...
static volatile int16_t pre_x[GTP_MAX_TOUCH] ={-1,-1,-1,-1,-1};
static volatile int16_t pre_y[GTP_MAX_TOUCH] ={-1,-1,-1,-1,-1};
//static volatile int16_t pre_x[GTP_MAX_TOUCH] ={-1};
//static volatile int16_t pre_y[GTP_MAX_TOUCH] ={-1};
